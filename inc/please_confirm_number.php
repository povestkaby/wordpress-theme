...
<?php /*	Согласно <strong>Постановлению Совета Министров Республики Беларусь от <a href = "https://news.tut.by/society/616915.html">23 ноября 2018 г.</a> №850</strong> "Об утверждении Положения о порядке предварительной идентификации пользователей интернет-ресурса, сетевого издания", с 1 декабря 2018 г. для того, чтобы пользователи нашего сайта могли писать вопросы, ответы, комментарии, им нужно указать номер телефона, на который придет СМС с кодом. После единовременного введения этого кода сайтом можно будет полноценно пользоваться, как и прежде. Данная процедура совершенно бесплатна.
	...
	<strong>Привяжите номер телефона</strong>! Ваш номер телефона не будет виден другим пользователям. Привязка номера используется только для защиты от спама. При проблемах с привязкой номера свяжитесь с технической поддержкой по номеру +375291998650<br><br>
<a name="form"></a>
<?php
	global $wpdb;
		...
		if(isset($_POST['phone']) AND validate_phone_number(htmlspecialchars($_POST['phone']))) {
			...
			if($existing_users){
				...
				if(sendRocketSMS(htmlspecialchars($_POST['phone']), $code)){
					...
					?>
					На ваш номер <strong><?php echo htmlspecialchars($_POST['phone']); ?></strong> отправлено смс с 5-ти значным кодом. Введите код из смс.
